...
async function stageApp() {
  ...
  await cp(path.join(webuiRoot, "electron", "main.cjs"), path.join(stageRoot, "main.cjs"));
  await cp(path.join(webuiRoot, "dist"), path.join(stageRoot, "WebUI", "dist"), { recursive: true });
  await cp(path.join(webuiRoot, "vendor"), path.join(stageRoot, "WebUI", "vendor"), { recursive: true });
  await cp(path.join(webuiRoot, "index.html"), path.join(stageRoot, "WebUI", "index.html"));
  await cp(path.join(webuiRoot, "styles.css"), path.join(stageRoot, "WebUI", "styles.css"));
  await cp(bundleRoot, path.join(stageRoot, "Examples", "WGSExtract"), { recursive: true });
  await cp(
    path.join(repoRoot, "Sources", "GUIForCLICore", "Resources", "BuiltinStrings"),
    path.join(stageRoot, "Sources", "GUIForCLICore", "Resources", "BuiltinStrings"),
    { recursive: true }
  );
}
