...
export function allControls(manifest) {
  return (manifest.pages ?? []).flatMap((page) =>
    (page.sections ?? []).flatMap((section) => section.controls ?? []),
  );
}
...
export function disabledReason(action, context) {
  if (!(action.disabledWhen ?? []).some((condition) => conditionMatches(condition, context))) {
    return undefined;
  }
  return action.disabledTooltip ? interpolate(action.disabledTooltip, context) : "This action is not available.";
}
...
export function checkedOptionsForContext(checkedOptions) {
  return Object.fromEntries(
    Object.entries(checkedOptions).map(([key, selected]) => [
      key,
      selected instanceof Set ? [...selected].sort().join(",") : [...(selected ?? [])].sort().join(","),
    ]),
  );
}
