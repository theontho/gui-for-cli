...
| C++ ImGui source | 2,819 | 2.82 | `Apps/ImGuiCpp/src/*.cpp` and `*.hpp`. |
| C++ ImGui CMake | 79 | 0.08 | `Apps/ImGuiCpp/CMakeLists.txt`. |
...
GUI_FOR_CLI_OFFLINE=1 /usr/bin/time -l Apps/ImGui/target/release/gui-for-cli-imgui \
  --bundle Examples/WGSExtract \
  --benchmark \
  --benchmark-full \
  --once

GUI_FOR_CLI_OFFLINE=1 /usr/bin/time -l Apps/ImGuiCpp/build/gui-for-cli-imgui-cpp \
  --bundle Examples/WGSExtract \
  --repo-root "$(pwd)" \
  --benchmark \
  --benchmark-full \
