...
The recommended architecture is: **Windows app = C# WinUI 3 + MVVM Toolkit + System.Text.Json models mirroring the manifest schema + Windows process services**. The **WebUI implementation should be treated as the primary behavioral reference** because `platform\typescript\src\shared\rendering.ts` and `platform\typescript\src\shared\localization.ts` already encode the JSON-native runtime behavior independently from Swift.[^5] Reuse the manifest files, localization keys, examples, and cross-platform test fixtures as the contract; avoid sharing Swift code, Swift binaries, or the Swift runtime with the Windows implementation.
...
|---|---|
| `platform\typescript\src\shared\rendering.ts` | Placeholder interpolation, rendered commands, optional arguments, missing placeholders, action visibility/disabled logic, condition evaluation, row hydration, data-source payload merge semantics, flat TOML parsing/serialization, numeric expression evaluation |
| `platform\typescript\src\shared\localization.ts` | TOML string-table parsing, localization label defaults, manifest localization, table merge order |
| `platform\typescript\src\client\view.ts` | Complete rendering behavior for text/path/dropdown/toggle/checkboxGroup/infoGrid/libraryList/configEditor, page/section/action/terminal layout, confirmation dialog behavior |
| `platform\typescript\src\client\operations.ts` | Data-source refresh, action execution, precheck workflow, file-state refresh, bundle-state persistence, config save flow |
| `platform\typescript\src\server\main.ts` | API surface that describes all runtime operations the app needs |
| `platform\typescript\src\server\action-runner.ts` | File-state values, disk precheck, action command execution behavior |
| `platform\typescript\src\server\config-store.ts` | Bundle state, config binding, TOML config load/save behavior |
| `platform\typescript\src\server\bundle-loader.ts` | Page-file loading, string-table layering, exit-code defaults |
| `platform\typescript\src\server\process-runner.ts` | Process execution semantics to preserve, with Windows process-tree behavior replaced by native Job Objects |
...
2. Port the platform-neutral behavior from `platform\typescript\src\shared\rendering.ts`: `interpolate`, `contextValue`, `renderedCommand`, `missingPlaceholders`, `conditionMatches`, `isActionVisible`, `disabledReason`, `hydrateRows`, `rowContext`, `applyDataSourcePayload`, `parseFlatToml`, `serializeFlatToml`, and `evaluateNumeric`.
3. Port the localization behavior from `platform\typescript\src\shared\localization.ts`: TOML parsing, label defaults, string-table layering, and deep manifest localization.
...
- `exp-platform\windows\GUIForCLIWindows.sln`
...
- platform-neutral builtin string resources consumed from `platform\apple\shared\Sources\GUIForCLICore\Resources\BuiltinStrings`
...
- Add `exp-platform\windows\dotnet\GUIForCLIWindows.Core`.
...
[^1]: Microsoft Learn, WinUI 3 overview: https://learn.microsoft.com/en-us/windows/apps/winui/winui3/.
[^2]: Microsoft Learn, Windows App SDK overview: https://learn.microsoft.com/en-us/windows/apps/windows-app-sdk/.
[^3]: Repository schema and split page layout: `docs/schema/manifest.schema.json`, `examples/WGSExtract/manifest.json`, and `examples/WGSExtract/pages/`.
[^4]: Swift Package Manager package model: `Package.swift`; Swift on Windows getting started: https://www.swift.org/getting-started/windows/.
[^5]: Existing macOS/iOS shared UI and core boundaries: `platform/apple/shared/app/`, `platform/apple/shared/Sources/GUIForCLICore/`, and `platform/apple/shared/Sources/GUIForCLICLI/`.
[^6]: Microsoft Learn, WPF overview: https://learn.microsoft.com/en-us/dotnet/desktop/wpf/overview/.
[^7]: Microsoft Learn, Windows Forms overview: https://learn.microsoft.com/en-us/dotnet/desktop/winforms/overview/.
[^8]: Microsoft Learn, MSIX overview: https://learn.microsoft.com/en-us/windows/msix/overview; Windows App SDK deployment guide: https://learn.microsoft.com/en-us/windows/apps/windows-app-sdk/deploy-unpackaged-apps.
[^9]: Microsoft Learn, WinUI 3 Gallery and controls: https://learn.microsoft.com/en-us/windows/apps/design/controls/.
[^10]: Data-driven UI schema examples: `docs/schema/manifest.schema.json` and `examples/WGSExtract/pages/`.
[^11]: Microsoft Learn, Windows App SDK self-contained deployment: https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/self-contained-deploy/deploy-self-contained-apps.
[^12]: Swift/C# interop should stay behind generated or process boundaries; current CLI/core separation is visible in `platform/apple/shared/Sources/GUIForCLICLI/` and `platform/apple/shared/Sources/GUIForCLICore/`.
[^13]: Current repository test/build surfaces: `Makefile`, `make.ps1`, `.github/workflows/`, `Tests/`, and `platform/typescript/package.json`.
[^14]: Current WebUI runtime and tests: `platform/typescript/src/`, `platform/typescript/tests/`, and `platform/typescript/package.json`.
[^15]: Microsoft Learn, WinUI layout panels: https://learn.microsoft.com/en-us/windows/apps/design/layout/.
[^16]: Microsoft Learn, accessibility in Windows apps: https://learn.microsoft.com/en-us/windows/apps/design/accessibility/accessibility.
[^17]: WinUI controls and data binding documentation: https://learn.microsoft.com/en-us/windows/apps/design/controls/ and https://learn.microsoft.com/en-us/windows/apps/develop/data-binding/.
[^18]: Microsoft Learn, app lifecycle for Windows apps: https://learn.microsoft.com/en-us/windows/apps/windows-app-sdk/applifecycle/applifecycle.
[^19]: Microsoft Learn, Windows console pseudoconsole overview: https://learn.microsoft.com/en-us/windows/console/creating-a-pseudoconsole-session.
...
[^21]: Microsoft Learn, CreateProcessW reference: https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessw.
[^22]: Windows implementation work is tracked in this repository under `exp-platform/windows/dotnet/GUIForCLIWindows/`, `exp-platform/windows/dotnet/GUIForCLIWindows.Core/`, and `exp-platform/windows/dotnet/GUIForCLIWindows.CoreTests/`.
[^23]: Microsoft Learn, package identity overview: https://learn.microsoft.com/en-us/windows/apps/desktop/modernize/grant-identity-to-nonpackaged-apps.
[^24]: Microsoft Learn, MSIX packaging tool and packaging guidance: https://learn.microsoft.com/en-us/windows/msix/packaging-tool/tool-overview.
[^25]: Microsoft Learn, UI Automation fundamentals: https://learn.microsoft.com/en-us/windows/win32/winauto/uiauto-uiautomationoverview.
