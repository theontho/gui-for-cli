...
- Windows measurements were taken on Windows benchmark hosts documented in `docs/ai/platforms/windows.md`; macOS measurements are from `docs/ai/platforms/apple-macos.md`.
...
| [#30](https://github.com/theontho/gui-for-cli/pull/30) | Open | Dioxus Native WebUI shell | Rust shell around the platform/typescript/Node/WebView2 architecture; smaller package than Tauri/Electron but still WebView2-heavy. |
...
See `docs/ai/gui-toolkit-macos-benchmark-run.md` for the dedicated benchmark record with commands, readiness definitions, and caveats.
...
| --- | ---: | ---: | --- |
| SwiftUI shared/macOS | 45 | 4,305 | `platform/apple/swiftui` is a tiny app entrypoint over `platform/apple/shared/app`, which contains bundle chrome, page/control renderers, data-source loading, setup orchestration, terminal/process handling, accessibility annotations, and reusable controls. |
| WebView shell | 1 | 312 | `platform/typescript/web/packagers/webview-shell/Shell.swift` is a compact custom macOS shell that starts the Node backend and hosts WebUI in WKWebView. |
| Flutter | 9 | 2,046 | `exp-platform/dart/flutter/lib/main.dart` hosts app state and routing, while `src/` contains widgets, bundle loading, startup benchmarking, localization, models, data-source execution, and command rendering helpers. |
| Slint | 3 | 795 | `exp-platform/rust/slint/src` keeps the Slint UI/action runner thin while shared Rust bundle, setup, terminal, and process logic lives under `exp-platform/rust/shared/src`. |
| Windows C# app/core/tests | 25 | 5,296 | `exp-platform/windows/dotnet/GUIForCLIWindows` contains WinUI shell/pages/controls, `exp-platform/windows/dotnet/GUIForCLIWindows.Core` contains bundle/runtime services, and `exp-platform/windows/dotnet/GUIForCLIWindows.CoreTests` covers the core layer. |
| WebUI browser/server/TUI | 47 | 7,807 | `platform/typescript/src` carries browser UI, server APIs, shared rendering/model code, TUI modules, localization, and tests. This is the largest reusable cross-surface codebase. |
| Tauri shell | 2 | 311 | `platform/typescript/web/packagers/tauri` is a thin Rust wrapper around the existing WebUI build plus bundled Node/runtime resources. |
