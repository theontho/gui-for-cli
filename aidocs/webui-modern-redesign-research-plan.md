...
| **Scope** | Full WebUI visual layer — `WebUI/styles.css`, `WebUI/index.html`, `WebUI/src/client/` |
...
| **Test surface** | CSS changes do not affect `make test-webui`; TS template changes require `npm --prefix WebUI test` |
...
| File | Role | Size |
|---|---|---|
| `WebUI/styles.css` | All visual rules — single source of truth for design | ~971 lines |
| `WebUI/index.html` | Shell HTML; loads Bootstrap Icons from jsDelivr CDN | 21 lines |
| `WebUI/src/client/app.ts` | Root render loop; full `innerHTML` replacement on every state change | 107 lines |
| `WebUI/src/client/view.ts` | HTML template string functions for nav, cards, forms, tables, modals | 426 lines |
| `WebUI/src/client/terminal.ts` | Terminal pane templates + tab/status management | 124 lines |
| `WebUI/src/client/events.ts` | DOM event binding, action handling, splitter drag | 210 lines |
| `WebUI/src/client/model.ts` | Shared helpers: `renderIcon`, `renderTooltip`, `renderLoadingBox` | 156 lines |
| `WebUI/src/client/state.ts` | `WebUIState` shape + localStorage hydration | 36 lines |
| `WebUI/src/client/icons.ts` | `emojiIconMap`, `bootstrapIconMap` | 90 lines |
| `WebUI/tests/rendering.test.mjs` | Node test runner — covers shared rendering/localisation logic, not CSS | — |
...
The current `:root` block at `WebUI/styles.css:1-38`[^2] defines:
...
Replace `WebUI/styles.css:1-38`[^2] with an expanded token block in this direction:
...
**Viewport meta** (`WebUI/index.html:5`[^17]):
...
**Acceptance:** `npm --prefix WebUI run check` passes; `make test-webui` passes; visual diff is intentionally minimal.

### Milestone 2 — Focus Rings + Transition Layer

**Files:** `WebUI/styles.css`
...
**Files:** `WebUI/styles.css`, `WebUI/index.html`
...
**Files:** `WebUI/styles.css`, `WebUI/index.html`, `WebUI/src/client/view.ts`, `WebUI/src/client/events.ts`, `WebUI/src/client/model.ts`
...
[^1]: `WebUI/src/client/app.ts:50-76` — full synchronous `innerHTML` replacement render loop; `bindEvents(bootstrap)` re-runs after every render.
[^2]: `WebUI/styles.css:1-38` — complete CSS custom property `:root` block and current token definitions.
[^3]: `WebUI/styles.css:7` — `--panel-raised` is defined as `color-mix(in srgb, CanvasText 3%, Canvas)`; subagent audit found no references elsewhere in the file.
[^4]: `WebUI/styles.css:59` — `button { border-radius: 7px }`.
[^5]: `WebUI/styles.css:357` — `input[type="text"] { border-radius: 6px }`.
[^6]: `WebUI/styles.css:296-301` — `.card` has border, radius, background, padding, and no shadow.
[^7]: `WebUI/styles.css:419` — `.table-wrap { border-radius: 8px }`.
[^8]: `WebUI/styles.css:789` — `.confirmation-modal { border-radius: 12px }`.
[^9]: `WebUI/styles.css:602` — `.pill { border-radius: 999px }`.
[^10]: `WebUI/styles.css:586` — `.floating-tooltip { box-shadow: 0 12px 36px rgb(0 0 0 / 20%) }`.
[^11]: `WebUI/styles.css:792` — `.confirmation-modal { box-shadow: 0 24px 80px rgb(0 0 0 / 30%) }`.
[^12]: `WebUI/styles.css` — subagent audit reported zero `transition:` properties and no `prefers-reduced-motion` block.
[^13]: `WebUI/styles.css:20` — `--accent: AccentColor`.
[^14]: `WebUI/styles.css:46` — `body { min-width: 760px }`.
[^15]: `WebUI/styles.css:77` — `.app-shell { grid-template-columns: var(--sidebar-width, 220px) 8px minmax(520px, 1fr) }`.
[^16]: `WebUI/styles.css:857-970` — the only responsive breakpoint is `@media (max-width: 900px)`.
[^17]: `WebUI/index.html:5` — viewport meta lacks `viewport-fit=cover`.
[^18]: `WebUI/styles.css:79,87,204,849,944` — `100vh` uses in app shell, sidebar, detail shell, loading screen, and mobile detail shell.
[^19]: `WebUI/styles.css:57-74` — base button block; no general `:focus-visible` rule.
[^20]: `WebUI/styles.css:67-69` — `button:hover:not(:disabled) { filter: brightness(0.98) }`.
[^21]: `WebUI/styles.css:711-723` — `.terminal-tab-close` has 18 px dimensions; compare WCAG 2.5.8 target-size minimum: https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html.
[^22]: `WebUI/styles.css:562-575` — `.tooltip` has 18 px dimensions.
[^23]: `WebUI/src/client/view.ts:55` — nav item active state uses class only; no `aria-current`.
[^24]: `WebUI/src/client/view.ts:409` — confirmation modal has `role="dialog" aria-modal="true" aria-labelledby="confirm-title"` but no focus trap in events.
[^25]: `WebUI/src/client/events.ts:51-58` and `WebUI/src/client/events.ts:89-94` — `window.prompt()` path prompts.
[^26]: `WebUI/styles.css:329` — `.form-row, .toggle-row { grid-template-columns: 190px minmax(220px, 1fr) minmax(0, 0) }`.
[^27]: `WebUI/src/client/model.ts:96` — tooltip rendered as `<span class="tooltip" tabindex="0" role="button" ...>i</span>`.
...
[^36]: `WebUI/styles.css:688-697` — `.terminal-tab-wrap.error/.warning/.success` status classes.
...
[^46]: `WebUI/styles.css:812-819` and `WebUI/styles.css:851-854` — spinner animation and keyframes.
...
[^49]: `WebUI/src/client/view.ts:331-337` — action button rendering uses `title`; icon-only actions need `aria-label`.
[^50]: `WebUI/src/client/view.ts:202-210` — `renderInfoGrid()` label and grid are not linked.
...
[^53]: `WebUI/styles.css:94-114` — `.sidebar-resizer` currently uses an 8 px visible track.
[^54]: `WebUI/styles.css:753-759` — `.terminal-log pre` monospace font stack.
[^55]: `WebUI/src/client/events.ts:155-163` — confirmation dialog input updates button disabled state imperatively without rerendering.
...
[^58]: `WebUI/styles.css:110-114` — existing `.sidebar-resizer:focus-visible` rule.
[^59]: `WebUI/styles.css:233-237` — existing `.terminal-resizer:focus-visible` rule.
