...
- This file is a same-machine cloud-agent companion to `aidocs/windows-benchmark.md`.
...
- The Electron package was produced with the equivalent direct `npm --prefix WebUI exec electron-packager -- ...` command after `.\make.ps1 package-electron` hit a Node `spawn EINVAL` error in this runner.
...
- Artifact: `WebUI\src-tauri\target\release\gui-for-cli-webui-tauri.exe` plus staged resources under `WebUI\src-tauri\target\release`
- Build: `npm --prefix WebUI run tauri:prepare-node`, then `cargo build --manifest-path WebUI\src-tauri\Cargo.toml --release --features bench-console`
...
- Launch target: `node WebUI\dist\tui\main.js --bundle Examples\WGSExtract --once --no-setup`
...
Scenario: production WebUI server only, launched as `node WebUI\dist\server\main.js --bundle Examples\WGSExtract --port <port>`, then polled until `GET /api/manifest` returned HTTP 200.
...
- Package command: `npm --prefix WebUI exec electron-packager -- WebUI\.cache\electron-package\app "GUI for CLI Electron" --platform=win32 --arch=x64 --out=out\windows-electron --overwrite --quiet`
