...
5. Raygui is the smallest native-rendered GUI prototype measured so far, with a 2.3 MB release executable and 915 KB compressed app-only payload. Its 255.8 ms content-ready marker is fast, but it still needs video-labeled visual startup capture before ranking it against Slint's perceived startup number.
...
8. **Raygui Rust app** as the smallest native-rendered prototype and a useful immediate-mode benchmark candidate after the spec-parity pass, pending visual startup capture and accessibility tradeoff review.
