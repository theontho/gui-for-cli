...
fn parse_args_from(current_dir: PathBuf, raw: impl Iterator<Item = String>) -> Result<Args> {
    ...
    while let Some(argument) = raw.next() {
        match argument.as_str() {
            ...
            "--version" | "-V" => {
                println!("gui-for-cli-gpui {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
            "--help" | "-h" => {
                print_help();
...
fn print_help() {
    println!(
        "Usage: gui-for-cli-gpui [--bundle PATH] [--repo-root PATH] [--locale CODE] [--check] [--benchmark] [--benchmark-full] [--once] [--benchmark-output PATH] [--version]"
    );
}
