...
use crate::control_text::data_source_values;
use crate::execution::{
...
pub struct GtkAppModel {
    ...
    pub(crate) data_source_action_errors: BTreeSet<String>,
...
impl GtkAppModel {
    pub fn load(args: Args) -> Result<Self> {
        ...
        let persisted_state = match load_state(&bundle_root) {
            Ok(state) => state,
            Err(error) => {
                startup_messages.push(format!("Could not load GTK4 state: {error:#}"));
    ...
    pub fn handle_terminal_action(&mut self, index: usize) {
        if let Some(TerminalAction::Cancel(id)) = self.terminal.tab_action(index)
            && let Err(error) = cancel_running_process(id, &self.running_processes)
        {
    ...
    pub fn set_control_value_by_id(&mut self, control_id: &str, value: String) {
        if let Some(control) = self.find_control(control_id) {
            self.set_control_value(&control, value);
        }
    }

    pub fn pick_control_path(&mut self, control_id: &str) {
        let Some(control) = self.find_control(control_id) else {
            return;
        };
...
pub fn label_from(labels: &BTreeMap<String, String>, key: &str) -> String {
