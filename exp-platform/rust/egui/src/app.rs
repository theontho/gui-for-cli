...
pub struct EguiApp {
    ...
    pub exit_code_reference: BTreeMap<i32, crate::exit_codes::ExitCodeReferenceView>,
...
impl EguiApp {
    pub fn load(args: Args) -> Result<Self> {
        ...
        let persisted_state = match load_state(&bundle_root) {
            Ok(state) => state,
            Err(error) => {
                startup_messages.push(format!("Could not load egui state: {error:#}"));
    ...
    pub fn emit_benchmark(&mut self, output_path: Option<&Path>) -> Result<()> {
        ...
        let full_feature_warm = full_feature_warm_ms
            ...
            .unwrap_or_default();
        let message = format!(
            "gfc-egui benchmark bundle_loaded_ms={:.1} ui_ready_ms={:.1}{full_feature_warm} pages={} controls={} actions={} setup_steps={} data_sources={} data_sources_loaded={} terminal_text_direction={}",
            self.loaded_ms,
            self.ready_ms,
            self.pages.len(),
            self.control_count,
    ...
    pub fn set_control_value(&mut self, control: &ControlView, value: String) {
    ...
    pub fn pick_control_path(&mut self, control: &ControlView) {
    ...
    pub fn log_data_source_error(&mut self, message: String) {
