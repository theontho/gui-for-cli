...
| `make worktree-setup BRANCH=my-feature` | Create a sibling developer worktree, copy local ignored dev config, and run the platform setup command (`.\make.ps1 setup` on Windows, `make setup` elsewhere). Set `WORKTREE_PATH=...` to override the location. |
...
`embeddedBundlePath` reads the bundle `manifest.json` and uses its configured app name or bundle directory name as the generated app display and product name. Shipped bundle-specific products keep that base name, such as `WGSExtract`; the macOS WebUI developer target is the exception and adds a `WebUI` suffix. Release packaging sets `bundleIdentifierName` so the macOS bundle identifier stays `dev.guiforcli.embed.<appname>` from the normalized base app name. Delete `tmp/app-identity.json` and regenerate after `cd platform/apple && ../../scripts/tuist.sh clean manifests` to return to the generic app identity.
