#!/usr/bin/env python3
"""Benchmark the staged Gio app by reading its startup metric lines."""
...
def main() -> int:
    ...
    if any("firstFrameRendered" not in run["metrics"] for run in runs):
        print("error: one or more Gio runs did not report firstFrameRendered", file=sys.stderr)
        return 1
    return 0
...
def median_metrics(runs: list[dict]) -> dict:
    ...
    medians = {
        f"{name}Ms": statistics.median(run["metrics"][name] for run in runs if name in run["metrics"])
        for name in names
    }
