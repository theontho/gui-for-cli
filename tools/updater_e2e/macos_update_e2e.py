...
def main() -> int:
    ...
    parser.add_argument("--surface", choices=("all", "swiftui", "tauri"), default="all")
    parser.add_argument("--skip-build", action="store_true", help="Reuse previously built old apps in work-dir.")
    parser.add_argument("--no-video", action="store_true", help="Skip screencapture video recording.")
    parser.add_argument("--video-seconds", type=int, default=90)
    args = parser.parse_args()

    if sys.platform != "darwin":
        raise SystemExit("macOS updater E2E tests must run on macOS.")

    surfaces = ["swiftui", "tauri"] if args.surface == "all" else [args.surface]
    ...
    if "tauri" in surfaces:
        app = old_tauri_app(args, release)
        results["tauri"] = run_update_flow(
            surface="tauri",
            app=app,
            expected_version=release.version,
...
def run_update_flow(
    *,
    ...
    video = work_dir / "videos" / f"{surface}-update.mov" if record else Path()
    if record:
    ...
    try:
        ...
        drive_update_until_version(app, expected_version, process_name, buttons, timeout=video_seconds)
    finally:
        stop_running_app(process_name)
...
def start_recording(video: Path, seconds: int) -> subprocess.Popen:
    return subprocess.Popen(["screencapture", "-v", "-V", str(seconds), "-k", str(video)])
...
def wait_for_process(process_name: str, timeout: int = 30) -> None:
    deadline = time.monotonic() + timeout
    script = f'''
tell application "System Events"
  if not (exists process "{process_name}") then return false
  tell process "{process_name}" to return (count of menu bars) > 0
end tell
'''
    while time.monotonic() < deadline:
        result = subprocess.run(["osascript", "-e", script], text=True, capture_output=True, check=False)
