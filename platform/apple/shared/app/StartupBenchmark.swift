...
@MainActor
enum StartupBenchmark {
  private static var reportedMetrics: Set<String> = []

  static func markWindowAppeared(since start: Date) {
    mark("window_appeared", since: start)
  }

  static func markContentInitialized(since start: Date) {
    mark("content_initialized", since: start)
  }

  private static func mark(_ metric: String, since start: Date) {
    ...
    guard
      ...
      !reportedMetrics.contains(metric)
    else {
      return
    }
    reportedMetrics.insert(metric)
    let elapsed = Date().timeIntervalSince(start) * 1000
    let message = String(format: "gfc-swiftui benchmark \(metric)_ms=%.1f", elapsed)
    print(message)
