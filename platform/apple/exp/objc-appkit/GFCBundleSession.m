...
+ (NSURL *)repoRootURL {
  ...
  if (sourceRoot.length > 0 && [NSFileManager.defaultManager fileExistsAtPath:[sourceRoot stringByAppendingPathComponent:@"Examples"]]) {
  ...
  for (NSURL *start in starts) {
    ...
    while (candidate.path.length > 1) {
      if ([NSFileManager.defaultManager fileExistsAtPath:[candidate.path stringByAppendingPathComponent:@"Examples"]]) {
        return candidate;
      }
