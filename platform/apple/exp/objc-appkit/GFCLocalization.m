...
+ (NSDictionary<NSString *, NSString *> *)loadStringTableWithRepoRoot:(NSURL *)repoRoot
  ...
  if (repoRoot != nil) {
    ...
    if (![locale isEqualToString:@"en"]) {
      NSString *fileName = [NSString stringWithFormat:@"strings.%@.toml", locale];
      NSString *localizedBuiltinStringsPath = [builtinStringsPath stringByAppendingPathComponent:fileName];
      [table addEntriesFromDictionary:[self tableAtURL:[repoRoot URLByAppendingPathComponent:localizedBuiltinStringsPath]]];
    }
  }
