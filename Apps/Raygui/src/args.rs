            ...
            "--benchmark-output" => {
                args.benchmark_output =
                    Some(PathBuf::from(raw.next().ok_or_else(|| {
                        anyhow!("--benchmark-output requires a path")
                    })?));
            }
            "--version" | "-V" => {
                println!("gui-for-cli-raygui {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
            "--help" | "-h" => {
                print_help();
...
fn print_help() {
    println!(
        "Usage: gui-for-cli-raygui [--bundle PATH] [--repo-root PATH] [--locale CODE] [--check] [--benchmark] [--once] [--benchmark-output PATH] [--version]"
    );
}
