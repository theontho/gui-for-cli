  ...
  while (index < lines.length) {
    ...
    if (rawValue.startsWith('"""')) {
      ...
      if (sameLineEnd >= 0) {
        ...
        if (!foundEnd) {
          throw FormatException('Unterminated multiline localization string: $key');
        }
      }
...
String localized(String value, Map<String, String> table) => table[value] ?? value;
...
BundlePage _localizePage(BundlePage page, Map<String, String> table) =>
    BundlePage(
      ...
      sections: page.sections.map((section) => _localizeSection(section, table)).toList(),
    );
...
PageSection _localizeSection(PageSection section, Map<String, String> table) =>
    PageSection(
      ...
      controls: section.controls.map((control) => _localizeControl(control, table)).toList(),
      actions: section.actions.map((action) => _localizeAction(action, table)).toList(),
    );
...
ControlSpec _localizeControl(ControlSpec control, Map<String, String> table) =>
    ControlSpec(
      ...
      columns: control.columns
          .map((column) => ListColumnSpec(id: column.id, title: localized(column.title, table)))
          .toList(),
      rows: control.rows.map((row) => _localizeRow(row, table)).toList(),
      items: control.items
          .map((item) => ListItemSpec(item.values.map((key, value) => MapEntry(key, localized(value, table)))))
          .toList(),
      rowTemplate: control.rowTemplate == null ? null : _localizeRow(control.rowTemplate!, table),
      rowActions: control.rowActions.map((action) => _localizeAction(action, table)).toList(),
      settings: control.settings.map((setting) => _localizeSetting(setting, table)).toList(),
    );

ListRowSpec _localizeRow(ListRowSpec row, Map<String, String> table) => ListRowSpec(
      id: row.id,
      ...
      tags: row.tags
          .map((tag) => TagSpec(id: tag.id, title: localized(tag.title, table), style: tag.style))
          .toList(),
    );
...
ConfigSettingSpec _localizeSetting(ConfigSettingSpec setting, Map<String, String> table) =>
    ConfigSettingSpec(
      id: setting.id,
      kind: setting.kind,
      key: setting.key,
...
String _unquoteKey(String key) =>
    key.startsWith('"') && key.endsWith('"') ? _unescapeTomlString(key.substring(1, key.length - 1)) : key;
