...
.PHONY: \
	...
	test test-webui test-flutter test-compose test-android test-gtk4 test-slint test-raygui test-imgui test-egui test-qt-qml test-avalonia test-fyne ax-smoke ax-smoke-ios ax-all \
	build-cli run-cli \
	...
	build-gtk4 run-gtk4 build-slint run-slint build-raygui run-raygui build-imgui run-imgui build-egui run-egui build-imgui-cpp run-imgui-cpp build-qt-qml run-qt-qml build-fyne run-fyne flutter flutter-build build-android run-compose-desktop build-compose-desktop launch-flutter-slint \
