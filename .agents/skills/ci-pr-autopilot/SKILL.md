...
When the user explicitly invokes this autopilot skill to babysit a PR, treat routine PR housekeeping as in scope unless the user narrowed the scope:

- If no PR exists for the current branch and the worktree has uncommitted changes, treat that as an explicit request to turn the current work into a new PR and then babysit it to completion.
- Resolve straightforward merge conflicts by merging the PR base branch into the PR branch with a normal merge commit, not a rebase.
- Mark a draft PR ready for review once the PR is otherwise in a mergeable, reviewable state.
- Rerun checks, refresh mergeability, and continue the loop after that housekeeping work.

Still stop and report instead of guessing when:

- Conflict resolution would require architectural changes, product decisions, schema or migration work, security-sensitive edits, or broad rewrites outside the PR scope.
- Draft status appears intentional for product or review reasons beyond routine housekeeping.
- The branch cannot be updated safely with a normal merge commit.

## Creating A PR From Uncommitted Work

When initial discovery finds no PR for the current branch and `git status --short` shows uncommitted changes, create a PR from that work instead of stopping, provided the user explicitly invoked autopilot without narrowing it to an existing PR only.

1. Confirm the branch is not the base branch (`main`, `master`, or the repository default branch). If it is, create and switch to a focused feature branch before committing.
2. Inspect the worktree with `git status --short`, `git diff --stat`, and focused diffs for changed files so you understand the change and avoid committing unrelated generated or private files.
3. Determine local verification commands and commit identity using the Initial Discovery rules before committing.
4. Run the relevant local verification for the current work. If verification fails, fix safe issues before creating the PR; stop if the failure cannot be fixed safely.
5. Stage only the intended work. Do not stage secrets, local identity files, caches, build outputs, or unrelated user changes.
6. Create a normal commit with a clear message and the required repository trailers, using the selected commit identity.
7. Push the branch to the appropriate remote without force-pushing.
8. Open a PR against the repository default branch or the branch requested by the user. Write a concise title and body that summarize the changes and list local verification that passed.
9. Continue with the normal Monitoring Loop for the new PR.

Do not create a PR from uncommitted work when:

- The worktree contains changes that appear unrelated to the requested work and cannot be separated safely.
- The current branch or remote state is ambiguous enough that committing would risk overwriting someone else's work.
- The local verification required for the change fails for a reason that cannot be fixed within scope.

## Initial Discovery

1. Identify the PR and repository:
   - If the user gave a PR URL, use `gh pr view <url> --json ...`.
   - Otherwise use `gh pr view --json ...` from the current branch. If no PR exists and there are uncommitted changes, follow "Creating A PR From Uncommitted Work".
...
2. If the PR is draft and the user explicitly invoked autopilot, treat that as a routine housekeeping task rather than an automatic blocker. Mark it ready once the branch is otherwise ready for review.
3. If the PR has merge conflicts and the conflict looks routine, merge the base branch into the PR branch, resolve conflicts conservatively, test, commit, push, and refresh PR state.
4. If new failing CI exists, diagnose, fix, test, commit, push, and wait.
5. If new actionable review feedback exists, fix or refuse with `(AI): Reason: ...`, commit, push if changed, and resolve handled threads.
6. If changes were pushed, wait for new CI results before merging.
7. If CI is pending, wait and poll at a reasonable interval. Avoid tight loops.
8. If PR state changes while you are working, re-read state before resolving threads or merging.
9. Continue until no failing/pending required checks, no unresolved material threads, no unhandled requested changes, and no new review comments remain.

Treat these as serious blockers and stop with a clear report:

- Merge conflicts that are not routine or cannot be resolved safely within the PR's existing scope.
- Draft PR only when the user did not explicitly invoke autopilot to handle routine PR housekeeping.
